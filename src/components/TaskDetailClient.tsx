...
export function TaskDetailClient({
  ...
  const { task, posts } = initialData;

  const getMediaImageCandidates = (post: any) => {
    if (!post) return [];
    if (post.resolved_image_url) return [post.resolved_image_url];
    if (post.resolved_image_url === null) return [];
    ...
    const pushCandidate = (subpath: string) => {
      if (!subpath) return;
      candidates.push(`http://data.abundis.com.mx/${subpath}`);
      candidates.push(`https://data.abundis.com.mx/${subpath}`);
    };

    // 1. Exported images by MUID
    if (task?.MUID && postMediaId) {
      pushCandidate(`media/exported_images/${task.MUID}/${postMediaId}_exported.jpg`);
      pushCandidate(`media/exported_images/${task.MUID}/${postMediaId}_exported.webp`);
    }

    // 2. Seed node / user_id + pk
    if (seedNode && post.user_id && post.pk) {
      pushCandidate(`media/${seedNode}/${post.user_id}_${post.pk}.jpg`);
      pushCandidate(`media/${seedNode}/${post.user_id}_${post.pk}.webp`);
    }

    // 3. Real username / pk
    if (realUser && post.pk) {
      pushCandidate(`media/${realUser}/${realUser}_${post.pk}.jpg`);
      pushCandidate(`media/${realUser}/${realUser}_${post.pk}.webp`);
    }

    // 4. Seed node / mediaName
    if (seedNode && mediaName) {
      pushCandidate(`media/${seedNode}/${mediaName}.jpg`);
      pushCandidate(`media/${seedNode}/${mediaName}.webp`);
    }
  ...
  const handleImgError = (e: React.SyntheticEvent<HTMLImageElement, Event>, post: any) => {
    ...
    if (nextStep < candidates.length) {
      ...
      // Si fallan todos los servidores de medios, ocultar la tarjeta del grid de forma limpia
  ...
  const gridPosts = useMemo(() => {
    return posts.filter((p) => {
      if (p.resolved_image_url === null) return false;
  ...
  return (
    <>
      ...
      {/* Interactive Posts Card with Instagram Grid & Table Switcher */}
      <Card className="shadow-sm mb-4">
        <Card.Header className="bg-light fw-bold fs-5 d-flex justify-content-between align-items-center flex-wrap gap-2 py-3">
          <div className="d-flex align-items-center gap-3">
            <span>Interactive Media Gallery ({posts.length} Posts)</span>
          </div>
          <div className="d-flex align-items-center gap-2">
            ...
            <div className="btn-group btn-group-sm" role="group" aria-label="View Mode Switcher">
              ...
              <Button
                ...
                className="fw-bold d-flex align-items-center gap-1"
              >
                <span>📋</span> Interactive Table ({posts.length})
              </Button>
            </div>
          </div>
        </Card.Header>
            ...
            <div className="p-3 bg-dark bg-opacity-10" style={{ maxHeight: '650px', overflowY: 'auto' }}>
              {gridPosts.length === 0 ? (
                <div className="text-center text-muted py-5">
                  <p className="mb-0">No image posts available for grid view</p>
